...
public class Point : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        ...
        Debug.Log("Point Mouse click on " + _position.x + ", " + _position.y + Pos.x); //Alone
        //Click --> Change color
        //By players
        if (Inner.GetComponent<SpriteRenderer>().color != Color.white) //If it is not white
            if (GameManager.Instance.GetGameState == GameManager.GameState.player1B || GameManager.Instance.GetGameState == GameManager.GameState.player2B) //but first turn
                Inner.GetComponent<SpriteRenderer>().color = Color.white; //Click --> Change color}
            else
